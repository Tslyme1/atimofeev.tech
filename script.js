document.addEventListener('DOMContentLoaded', () => {
  ...
  const dict = {
    ...
    'Декомпозиция': 'Breakdown',
    'Категории авторов': 'Creator categories',
    'Провёл исследование': 'Ran research',
    'Спроектировал решение': 'Designed the solution',
    'Запуск, тестирование, выводы': 'Launch, testing, takeaways',
    'Проектирование второй версии': 'Designing the second version',
    'Гипотезы развития': 'Growth hypotheses',
    'Подробнее': 'Learn more',
    'Релиз 1': 'Release 1',
    'Релиз 2': 'Release 2',
    'Успеха': 'Success rate',
    'Медианное время': 'Median time',
    'Не дошёл до заявки': 'Did not reach the form',
    'Дошёл до заявки': 'Reached the form',
    'Результаты': 'Results',
    ...
    // Monetization case (rebuild, node 365:485)
    'О продукте': 'About the product',
    'Исследование авторов': 'Creator research',
    'CJM авторов': 'Creator CJM',
    'Гипотезы и приоритизация': 'Hypotheses and prioritization',
    'Решение 1': 'Solution 1',
    'Тестирование и результат 1 решения': 'Testing and results of solution 1',
    'Решение 2': 'Solution 2',
    'Общие результаты': 'Overall results',
    'Мобильная студия — это мобильное приложение для авторов контента, которое позволяет управлять своим каналом RUTUBE. Пользователи могут загружать видео, проводить прямые эфиры, анализировать статистику канала и взаимодействовать с аудиторией': 'Mobile Studio is a mobile app for content creators that lets them manage their RUTUBE channel. Users can upload videos, run livestreams, analyze channel stats, and engage with their audience',
    'тыс+': 'K+',
    'Скачиваний': 'Downloads',
    'млн.': 'M',
    'Видео загрузили авторы': 'Videos uploaded by creators',
    'Монетизация — один из ключевых драйверов удержания авторов в Мобильной Студии RUTUBE: чем больше авторов зарабатывает на контенте, тем выше их мотивация загружать видео, а значит — растёт вотчтайм и рекламная выручка RUTUBE': 'Monetization is one of the key drivers of creator retention in RUTUBE\'s Mobile Studio: the more creators earn from content, the more motivated they are to upload videos — which grows watch time and RUTUBE\'s ad revenue',
    'Бизнес-цель — увеличение доли авторов-партнеров': 'Business goal — increase the share of partner creators',
    'Обратился к аналитике — 24% авторов мобильной студии подходят под условия монетизации, но заявку на нее не подавали': 'Checked the analytics — 24% of Mobile Studio creators meet the monetization conditions but never submitted an application',
    'Проблема заключалась в том, путь автора к монетизации в Мобильной Студии RUTUBE не существовал как сценарий': 'The problem: the path to monetization inside RUTUBE\'s Mobile Studio didn\'t exist as a scenario at all',
    'Дерево метрик Rutube': 'RUTUBE metrics tree',
    'Моб. студия до монетизации': 'Mobile Studio before monetization',
    'Кто целевой сегмент': 'Who the target segment is',
    'Поднял разрез по типам: 88% авторов подходящих под условия — самозанятые. Официально зарегистрированы, данные уже где то зарегистрированы, но в заявке их каждый раз вводят вручную': 'Broke it down by type: 88% of eligible creators are self-employed (“samozanyatye”). They\'re officially registered and their data already exists somewhere, yet the application makes them re-enter it by hand every time',
    'Почему не доходят до заявки': 'Why they don\'t reach the application',
    'Разобрал тикеты поддержки и регулярные исследования пользователей Мобильной Студии, часть авторов не знают:': 'Went through support tickets and ongoing Mobile Studio user research — some creators simply don\'t know:',
    'Может ли его контент монетизироваться вообще': 'Whether their content can be monetized at all',
    'Какие условия надо выполнить': 'What conditions they need to meet',
    'Что заявка на монетизацию вообще существует и что в ней требуется': 'That a monetization application even exists, or what it requires',
    'Что с теми кто доходит до заявки': 'What happens to those who reach the application',
    'Провел несколько глубинных исследований с авторами-партнерами о том как они заполняли заявку (в веб версии студии она существовала), основные озвученные проблемы:': 'Ran several in-depth interviews with partner creators about filling out the application (it existed in Studio\'s web version at the time) — the main problems they raised:',
    'Большая ручная форма требует много времени на заполнение и отпугивает': 'A long manual form takes too much time to fill in and puts people off',
    'Причины отказа в ряде кейсов не ясны, не категоризованы': 'Rejection reasons are often unclear and uncategorized',
    'Синканулись с модерацией, собрали данные:': 'Synced with moderation and gathered data:',
    'Топ причин отказа на заявку: ошибки в полях заполнения, недостаточно документов или данных': 'Top rejection reasons: field-entry errors, missing documents or data',
    'Ручная проверка вручную заполненных данных отнимает много времени': 'Manually checking manually entered data takes a lot of time',
    '88% авторов самозанятые, их данные уже где-то есть': '88% of creators are self-employed — their data already exists somewhere',
    'Для авторов непонятны условия монетизации': 'Monetization conditions aren\'t clear to creators',
    'Существующая форма сложная как для авторов так и для модерации': 'The existing form is hard both for creators and for moderation',
    'Разбил сценарий автора на части:': 'Broke the creator\'s journey into stages:',
    'Не знает о монетизации': 'Doesn\'t know about monetization',
    'Знает условия': 'Knows the conditions',
    'Выполнил условия': 'Met the conditions',
    'Подал заявку': 'Submitted the application',
    'Заявка одобрена/отклонена': 'Application approved/rejected',
    'Цель': 'Goal',
    '— не «больше заявок», а больше заявок, которые проходят модерацию с первого раза': '— not “more applications,” but more applications that pass moderation on the first try',
    'Целевые метрики:': 'Target metrics:',
    '% Авторов дошедших до заявки': '% of creators who reach the application',
    '% Авторов зашедших в заявку и отправивших её': '% of creators who open the application and submit it',
    '% Заявок, одобренных с первой попытки': '% of applications approved on the first try',
    'Схема становления автора партнёром': 'How a creator becomes a partner',
    'Собрали технические и бизнес ограничения, на которые опирались при формулировании гипотез и проектировании сценариев': 'Gathered the technical and business constraints that shaped the hypotheses and scenario design',
    'Бэк заявки один на веб и мобилку, переписывать под мобильный клиент нет возможности': 'The application backend is shared between web and mobile — rewriting it for the mobile client wasn\'t an option',
    'Сроки релиза и ресурс разработки очень ограничены': 'Release timeline and dev resources were very limited',
    'Требования к заявке диктует комплаенс, дизайн их не упрощает — только объясняет': 'Compliance dictates the application requirements — design can\'t simplify them, only explain them',
    '88% авторов мобильной студии подходящих под условия — самозанятые, их данные уже есть в госсистемах, но интеграции на текущий момент нет': '88% of eligible Mobile Studio creators are self-employed — their data already exists in government systems, but there\'s no integration yet',
    'Заявку подают три типа авторов — самозанятые, ИП и физлица. Состав документов и причины отказов у них разные': 'Three types of creators submit applications — self-employed, sole proprietors, and individuals. Their required documents and rejection reasons all differ',
    'Сформулировал проблемы по категориям авторов (Не дошел/Дошел до заявки) опираясь на исследование и CJM, на основе этого составил и приоритезировал гипотезы по решению этих проблем': 'Framed the problems by creator category (didn\'t reach the application / reached it), based on the research and CJM, then built and prioritized hypotheses to solve them',
    'Авторы, которые не дошли до заявки': 'Creators who didn\'t reach the application',
    'Проблема 1': 'Problem 1',
    'Автор не знает, что монетизация вообще существует и доступна ему': 'The creator doesn\'t know monetization exists or is available to them',
    'В Студии нет точки входа. Узнать можно только извне или от поддержки': 'There\'s no entry point in Studio. You can only find out from outside or from support',
    'Гипотезы по решению': 'Hypotheses to solve it',
    'Проблема 2': 'Problem 2',
    'Автор знает про монетизацию, но считает, что не подходит': 'The creator knows about monetization but assumes they don\'t qualify',
    'Условия сформулированы юридически и лежат вне продукта. Автор оценивает себя пессимистично и даже не проверяет': 'The conditions are written in legal language and live outside the product. Creators assume the worst and never even check',
    'Авторы, которые дошли до заявки': 'Creators who reached the application',
    'Автор доходит до заявки, но она выглядит как большая непонятная анкета, и он откладывает': 'The creator reaches the application, but it looks like a long, confusing form, so they put it off',
    'Одна длинная страница, непонятно, сколько это займёт и что понадобится': 'One long page — unclear how long it\'ll take or what\'s needed',
    'Автор начинает заполнять, прерывается и не возвращается': 'The creator starts filling it in, gets interrupted, and never comes back',
    'Черновика нет, возврат означает начать сначала': 'There\'s no draft — coming back means starting over',
    'Проблема 3': 'Problem 3',
    'Автор заполняет и отправляет, но получает отказ по данным': 'The creator fills it in, submits it, and gets rejected over their data',
    'Ошибка в поле или нехватка документа выясняется через недели, а не в момент ввода': 'A field error or missing document surfaces weeks later, not at the moment of entry',
    'Перед проектированием мы не знали где лучше разместить точку входа в страницу монетизации, на которой будут находиться:': 'Before designing, we didn\'t know where best to place the entry point to the monetization page, which would hold:',
    'Для авторов не партнеров — условия и требования к монетизации, прогресс до нее': 'For non-partner creators — monetization conditions and requirements, and progress toward them',
    'Для авторов партнеров — баланс и аналитика по выплатам': 'For partner creators — balance and payout analytics',
    'Если авторы не будут её замечать, то это снизит конверсию в переход, как следствие уменьшит воронку по дохождению до заявки/получению выплат': 'If creators don\'t notice it, that lowers click-through, which in turn shrinks the funnel to the application and to getting paid',
    'Провели ресерч с помощью тепловой карте на pathway, по тому где больше заметно': 'Ran research using a heatmap on the pathway to see where it stood out most',
    'База: 426 респондентов': 'Base: 426 respondents',
    'Аудитория: авторы RUTUBE': 'Audience: RUTUBE creators',
    'У 84% не подключена монетизация': '84% don\'t have monetization connected',
    '19.5 с.': '19.5 sec.',
    '20.48 с.': '20.48 sec.',
    'В Nav bar': 'In the nav bar',
    'В Tab bar': 'In the tab bar',
    'Далее сделал дизайн каждого из шагов, который проходит автор перед тем как стать партнером. Основные сущности:': 'Next, designed every step a creator goes through before becoming a partner. Core entities:',
    'Точка входа в монетизацию': 'Entry point to monetization',
    'Прогресс, показывающий сколько автору осталось до подачи заявки на монетизацию': 'Progress showing how far the creator is from submitting the monetization application',
    'Точка входа в заявку': 'Entry point to the application',
    'Статусы обработки заявки': 'Application status states',
    'Прогресс на главной и в монетизации': 'Progress on the home screen and in monetization',
    'Точка входа в заявку → вебвью (Переход на веб версию студии)': 'Entry point to the application → webview (redirect to Studio\'s web version)',
    'Статус обработки — успешно': 'Processing status — success',
    'Статус обработки — отклонено': 'Processing status — rejected',
    'Релиз 1 решения сделал ровно то, на что был рассчитан: авторы узнали о монетизации и пошли в форму. Но': 'Release 1 did exactly what it was designed to do: creators learned about monetization and went to the form. But',
    'форма не сумела обработать этот поток должным образом': 'the form couldn\'t handle that flow properly',
    'Авторов дошедших до заявки': 'Creators who reached the application',
    'Сквозная: открыл форму → отправил заявку': 'End-to-end: opened the form → submitted the application',
    'Заявок одобрено с первой попытки': 'Applications approved on the first try',
    'мин': 'min',
    'Среднее время прохождения заявки': 'Average time to complete the application',
    'Повторная подача после отказа': 'Resubmission after rejection',
    'Я провел серию глубинок с авторами, которые заполнили заявку в рамках 1 решения, какие у него оказались трейдоффы:': 'I ran a series of in-depth interviews with creators who filled out the application under solution 1 — here\'s what its trade-offs turned out to be:',
    'Пользователь устал заполнять — а у нас нет возможности сохранить черновик, только заполнять с нуля': 'Users got tired of filling it in — and there was no way to save a draft, only start over',
    'Сессия периодически прерывалась по инициативе пользователя/ошибкам бека — пользователь отваливался на середине процесса и недозаполнял': 'Sessions kept getting interrupted, by the user or by backend errors — people dropped off mid-flow and left it half-filled',
    'Прерывание опыта — внутри вебвью находится совершенно другой интерфейс, отличный от мобильной студии → увеличивается количество отказов': 'A broken experience — the webview has a completely different interface from Mobile Studio → more drop-offs',
    'Нашей команде удалось согласовать возможность интеграции/закупки Госуслуг, это стало большой возвожности для роста показателей одобрения заявок и проектировании нативной пошаговой формы': 'Our team managed to get sign-off on integrating with Gosuslugi (Russia\'s public-services portal) — a big opportunity to grow approval rates, and the basis for designing a native step-by-step form',
    'Одной из задач было создать понятный степпер, который отобразит автору (внутри заявки и на главном экране) информацию о том, на каком шаге он находится и что он уже прошел': 'One task was to design a clear stepper that shows the creator (inside the application and on the home screen) which step they\'re on and what they\'ve already completed',
    'Вариант степпера': 'Stepper variant',
    'Вариант 1': 'Variant 1',
    'Вариант 2': 'Variant 2',
    'Вариант 3': 'Variant 3',
    'Вариант 4': 'Variant 4',
    'Я спроектировал несколько вариантов, которые проверил на коридорном тестировании': 'I designed several variants and tested them in hallway testing',
    '— счётчик у заголовка плюс бар у кнопки. «1 из 3» интерпретируется однозначно, однако счётчик и полоса дублируют друг друга и разнесены по разным концам экрана, надпись «1 из 3» конкурирует с заголовком за внимание': '— a counter by the title plus a bar by the button. “1 of 3” reads unambiguously, but the counter and the bar duplicate each other and sit at opposite ends of the screen; “1 of 3” also competes with the title for attention',
    '— только сегменты. Занимает минимум места, однако число шагов приходится пересчитывать глазами, а закрашенный сегмент читается двояко: «этот шаг пройден» или «я сейчас здесь»': '— segments only. Takes up minimal space, but you have to count the steps by eye, and a filled segment reads two ways: “this step is done” or “I\'m here now”',
    '— объединенный 1 и 2 варианты, отсутствует дублирование, однако занимает больше места, чем остальные, и разваливается после пяти-шести шагов': '— variants 1 and 2 combined. No duplication, but it takes up more space than the others and breaks down after five or six steps',
    '— совмещает счётчик и прогресс в одном элементе: номер всегда там, где взгляд, но бегунок на полоске может читаться как элемент слайдера, а форма — перематываемой': '— combines the counter and progress into one element: the number is always where your eye lands, but the dot on the track can read as a slider control, making the form seem scrubbable',
    'Выяснилось, что наиболее очевидным вариантом оказался третий. Люди смогли быстрее всего определить на каком шаге они находятся и сколько им еще осталось': 'Turned out the third variant was the most obvious one. People identified their current step and how much was left fastest with it',
    'После спроектировал редизайн заявки, который поможет улучшить опыт и довести пользователя до её одобрения быстрее и проще:': 'Then redesigned the application to improve the experience and get users to approval faster and more easily:',
    'Вынес выбор типа автора вперёд и объяснил его. Было: сухой список — самозанятый / ИП / физлицо. Люди останавливались и спрашивали «а мне какой и для чего?»': 'Moved the creator-type choice up front and explained it. Before: a bare list — self-employed / sole proprietor / individual. People stalled and asked, “which one am I, and why?”',
    'Переработал большую форму в последовательную с шагами: Создание кошелька → Авторизация в госуслугах → Отправка данных → Ожидание модерации': 'Reworked the long form into a sequence of steps: Create a wallet → Sign in to Gosuslugi → Submit data → Await moderation',
    'Черновик заявки': 'Application draft',
    'Перед запуском провел исследование на 9 авторов': 'Before launch, ran research with 9 creators',
    'По типу:': 'By type:',
    'самозанятые': 'self-employed',
    'По опыту:': 'By experience:',
    'те, кто уже подавал заявку': 'those who had already applied',
    'Результаты:': 'Results:',
    'все респонденты дошли до конца, новых блокеров не нашли': 'all respondents made it to the end, no new blockers found',
    'Принято решение запускать пошаговую форму в релиз': 'Decided to ship the step-by-step form',
    'Открыл форму': 'Opened the form',
    '→ отправил заявку': '→ submitted the application',
    '16 мин': '16 min',
    '0% — Черновика нет': '0% — no draft',
    'Возврат к незавершённой заявке': 'Return to an unfinished application',
    '9 мин': '9 min',
    'По данным аналитики в Мобильной студии самая высокая доля партнеров среди остальных продуктов': 'According to analytics, Mobile Studio has the highest share of partners among the other products',
    'Как планируем дальше развивать функционал': 'How we plan to keep developing the feature',
    'Перенос сценария на ИП и юрлиц.': 'Extending the flow to sole proprietors and legal entities.',
    'Мы оптимизировали под самозанятых как под массовый сегмент, ИП и юрлица — дополнительная точка роста': 'We optimized for self-employed creators as the mass segment — sole proprietors and legal entities are an additional growth point',
    'Предпроверка контента до подачи.': 'Pre-check content before submission.',
    'Автор узнаёт о нарушении до того, как потратил время на заявку. Шаг зависит от модерации': 'The creator learns about a violation before spending time on the application. This step depends on moderation',
    'Уведомление о смене статуса заявки.': 'Notification when the application status changes.',
    'Сейчас автор узнаёт об отказе, только когда сам зайдёт в приложение или посмотрит электронную почту': 'Right now creators only learn about a rejection by opening the app themselves or checking email',
    'Прогноз выполнения условий в виджете.': 'A forecast for meeting conditions in the widget.',
    '«При текущем темпе вы выполните условие примерно через N дней». Дополнительная мотивация, но технически дорого': '“At your current pace, you\'ll meet this condition in about N days.” Extra motivation, but technically expensive',
  };
